...
bool MarkerDetector::locateFullStroke1D(
    const uint8_t* gray, int threshold,
    int expected_id, int expected_rotation,
    float center_y_px, float side_px,
    StrokeMatch& out) const {
    out = StrokeMatch{};
    if (!gray || side_px < appcfg::kMinMarkerSidePx * 0.75f) return false;

    const float side = clampf(
        side_px,
        static_cast<float>(appcfg::kMinMarkerSidePx),
        40.0f);
    const int half = static_cast<int>(ceilf(0.5f * side)) + 2;
    if (half >= _width / 2) return false;
    ...
    auto evaluate = [&](int cx, int cy) -> Eval {
        Eval e;
        if (cx - half < 1 || cx + half >= _width - 1 ||
            cy - half < 1 || cy + half >= _height - 1) {
            return e;
        }
        ...
        for (int row = 0; row < 6; ++row) {
            const float fy =
                ...
                (((static_cast<float>(row) + 0.5f) / 6.0f) - 0.5f) * side;
            const int y = static_cast<int>(lroundf(fy));
            if (y < 0 || y >= _height) return e;

            for (int col = 0; col < 6; ++col) {
                const float fx =
                    static_cast<float>(cx) +
                    (((static_cast<float>(col) + 0.5f) / 6.0f) - 0.5f) * side;
                const int x = static_cast<int>(lroundf(fx));
                if (x < 0 || x >= _width) return e;

                const int v = gray[y * _width + x];
                const bool white = v >= threshold;
                const int margin = abs(v - threshold);
                if (margin < 12) ++uncertain;

                if (row == 0 || row == 5 || col == 0 || col == 5) {
                    if (!white) ++border_black;
                } else {
                    payload = static_cast<uint16_t>(
                        (payload << 1) | (white ? 1u : 0u));
                }
            }
        }
        ...
        int rotation = -1;
        int hamming = 99;
        aruco4x4::match(payload, expected_id, 16, &rotation, &hamming);

        const int border_miss = 20 - border_black;
        const float rotation_penalty =
            rotation == expected_rotation ? 0.0f : 8.0f;
        e.score =
            5.0f * hamming +
            1.5f * border_miss +
            0.15f * uncertain +
            rotation_penalty;
        e.hamming = hamming;
        e.border_black = border_black;
        e.rotation = rotation;
        return e;
    };

    int best_x = 0;
    int best_y = static_cast<int>(lroundf(center_y_px));
    Eval best;

    const int base_y = static_cast<int>(lroundf(center_y_px));
    for (int dy = -appcfg::kStrokeYSearchPx;
         dy <= appcfg::kStrokeYSearchPx;
         dy += appcfg::kStrokeYStepPx) {
        const int cy = base_y + dy;
        for (int cx = half; cx < _width - half;
             cx += appcfg::kStrokeCoarseStepPx) {
            const Eval e = evaluate(cx, cy);
    ...
    if (best.score >= 1.0e8f) return false;

    const int coarse_x = best_x;
    const int coarse_y = best_y;
    for (int cy = coarse_y - 1; cy <= coarse_y + 1; ++cy) {
        for (int cx = coarse_x - appcfg::kStrokeCoarseStepPx;
             cx <= coarse_x + appcfg::kStrokeCoarseStepPx; ++cx) {
            const Eval e = evaluate(cx, cy);
            if (e.score < best.score) {
                best = e;
                best_x = cx;
    ...
    out.border_black = best.border_black;
    out.rotation = best.rotation;

    out.valid =
        best.rotation == expected_rotation &&
        best.hamming <= appcfg::kStrokeMaxHamming &&
        best.border_black >= appcfg::kStrokeMinBlackBorderCells &&
        best.score <= appcfg::kStrokeMaxScore;
    return out.valid;
}
